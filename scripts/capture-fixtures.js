...
const MSX_PACKAGE = "@microsoft/msx-mcp-server@latest";
const OIL_PACKAGE = "@jinlee794/obsidian-intelligence-layer@latest";
...
async function captureCrm(flags) {
  ...
  try {
    conn = await connectMcpServer(
      "msx-crm",
      "npx",
      ["-y", "--registry", NPM_REGISTRY, MSX_PACKAGE],
    );
  } catch (err) {
    console.error("❌ Failed to connect to MSX CRM server:", err.message);
    console.error("   Make sure you've run `az login` and can access the remote package registry.");
    return results;
  }
...
async function captureOil(flags) {
  ...
  try {
    conn = await connectMcpServer(
      "oil",
      "npx",
      ["-y", "--registry", NPM_REGISTRY, OIL_PACKAGE, "mcp"],
      { OBSIDIAN_VAULT_PATH: process.env.OBSIDIAN_VAULT_PATH },
    );
  } catch (err) {
    console.error("❌ Failed to connect to OIL server:", err.message);
    console.error("   Check OBSIDIAN_VAULT_PATH in .env and verify access to the remote package registry.");
    return results;
  }

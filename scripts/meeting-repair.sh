...
# meeting-repair.sh
#
# Runs the deterministic meeting brief repair prompt and validates the artifact.
...
if [ -z "$MEETING_NAME" ] || [ -z "$MEETING_FILE_SLUG" ]; then
  echo "[meeting-repair] ERROR: set MEETING_NAME and MEETING_FILE_SLUG env vars."
  echo "Example:"
  echo "  MEETING_NAME='Contoso QBR' MEETING_FILE_SLUG='contoso-qbr' npm run meeting:repair"
  exit 2
fi
...
if [ ! -x "$COPILOT_BIN" ] && ! command -v "$COPILOT_BIN" > /dev/null 2>&1; then
  echo "[meeting-repair] ERROR: copilot CLI not found"
  exit 1
fi

PROMPT_FILE="$REPO_DIR/.github/prompts/meeting-brief-repair.prompt.md"
if [ ! -f "$PROMPT_FILE" ]; then
  echo "[meeting-repair] ERROR: prompt file not found: $PROMPT_FILE"
  exit 1
fi
...
LOG_PATH="/tmp/meeting-brief-repair-$TARGET_DATE-$MEETING_FILE_SLUG.log"
...
bash "$REPO_DIR/scripts/validate-meeting-brief.sh" \
  --vault "$VAULT_DIR" \
  --date "$TARGET_DATE" \
  --meeting-file "$MEETING_FILE_SLUG.md"

echo "[meeting-repair] PASS: repaired and validated $TARGET_DATE-$MEETING_FILE_SLUG.md"
echo "[meeting-repair] Log: $LOG_PATH"
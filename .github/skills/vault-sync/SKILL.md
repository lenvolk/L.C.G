---
name: vault-sync
description: "Unified CRM→vault sync and hygiene skill. Six modes: opportunity sync, milestone sync, people sync (batch CRM + ad-hoc create from context), customer hygiene, task sync, and project sync (create + hygiene). Direction is always CRM→vault (one-way). Handles post-workflow auto-capture, on-demand batch sync, People note correlation, Agent Insights consolidation, task activity logging, and project note creation and standardization. Triggers: sync opportunity, opp sync, pipeline to vault, save opportunity, milestone sync, sync milestones, milestone vault sync, sync people, people sync, deal team people, create person, new people note, customer hygiene, clean up customer, consolidate insights, sync tasks, task sync, task activity log, vault capture opportunity, opportunity vault sync, deal team to vault, create project, new project, project hygiene, project sync, fix project notes, missing meetings dataview."
...
- **powerbi-billed-pipeline-hygiene** → this skill (Mode 1 batch, after hygiene review)
